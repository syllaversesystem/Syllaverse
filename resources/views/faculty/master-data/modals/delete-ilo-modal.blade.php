{{-- Delete ILO Modal (Faculty) --}}
<div class="modal sv-faculty-dept-modal" id="deleteIloModal" tabindex="-1" aria-labelledby="deleteIloModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <style>
        #deleteIloModal { --sv-bg:#FAFAFA; --sv-bdr:#E3E3E3; --sv-danger:#CB3737; }
        #deleteIloModal .modal-content{ border-radius:16px; border:1px solid var(--sv-bdr); background:#fff; box-shadow:0 10px 30px rgba(0,0,0,.08), 0 2px 12px rgba(0,0,0,.06); overflow:hidden; }
        #deleteIloModal .modal-header{ border-bottom:1px solid var(--sv-bdr); background: var(--sv-bg); }
        #deleteIloModal .modal-title{ color: var(--sv-danger); }
      ...
      <div class="modal-body">
        <div class="text-center mb-4">
          ...
          <h6 class="fw-semibold mb-2">Delete ILO</h6>
          <p class="text-muted mb-0">Are you sure you want to permanently delete this ILO?</p>
        </div>

        <div class="bg-light rounded-3 p-3 mb-2">
          <div class="small text-muted mb-1">You are about to delete:</div>
          <div class="fw-semibold mb-0" id="deleteIloTitle">Loading...</div>
          <div class="small text-muted mt-1" id="deleteIloDesc"></div>
        </div>
      ...
      <div class="modal-footer">
        ...
        <form id="deleteIloForm" method="POST" class="d-inline" action="#">
          @csrf
          @method('DELETE')
          <input type="hidden" id="deleteIloId" name="id" value="">
          <button type="submit" class="btn btn-danger" id="deleteIloSubmit"><i data-feather="trash-2"></i> Delete</button>
        </form>
      </div>

      <style>
        #deleteIloModal.modal-static .modal-dialog { transform: scale(1.02); transition: transform .2s ease-in-out; }
        #deleteIloModal.modal-static .modal-content { box-shadow: 0 8px 24px rgba(0,0,0,.12), 0 4px 12px rgba(0,0,0,.08); }
      </style>
    </div>
  </div>
</div>
