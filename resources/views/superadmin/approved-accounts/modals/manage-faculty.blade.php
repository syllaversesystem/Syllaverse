{{-- 
-------------------------------------------------------------------------------
* File: resources/views/superadmin/approved-accounts/modals/manage-faculty.blade.php
* Description: Clean faculty management modal - moved to Approved Accounts module
-------------------------------------------------------------------------------
--}}
...
{{-- ░░░ START: Revoke Faculty Confirmation Modal (Themed) ░░░ --}}
<div class="modal fade sv-faculty-dept-modal" id="revokeFacultyModal-{{ $faculty->id }}" tabindex="-1" aria-labelledby="revokeFacultyModalLabel-{{ $faculty->id }}" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <style>
        #revokeFacultyModal-{{ $faculty->id }} { --sv-bg:#FAFAFA; --sv-bdr:#E3E3E3; --sv-danger:#CB3737; }
        #revokeFacultyModal-{{ $faculty->id }} .modal-content{ border-radius:16px; border:1px solid var(--sv-bdr); background:#fff; box-shadow:0 10px 30px rgba(0,0,0,.08), 0 2px 12px rgba(0,0,0,.06); overflow:hidden; }
        #revokeFacultyModal-{{ $faculty->id }} .modal-header{ border-bottom:1px solid var(--sv-bdr); background: var(--sv-bg); padding:.85rem 1rem; }
        #revokeFacultyModal-{{ $faculty->id }} .modal-title{ color: var(--sv-danger); font-weight:600; font-size:1rem; display:inline-flex; align-items:center; gap:.5rem; }
        #revokeFacultyModal-{{ $faculty->id }} .modal-title i, #revokeFacultyModal-{{ $faculty->id }} .modal-title svg { width:1.05rem; height:1.05rem; }
        #revokeFacultyModal-{{ $faculty->id }} .btn-danger{ background:#fff; border:none; color: var(--sv-danger); transition: all .2s ease; display:inline-flex; align-items:center; gap:.5rem; padding:.5rem 1rem; border-radius:.375rem; }
        #revokeFacultyModal-{{ $faculty->id }} .btn-danger:hover, #revokeFacultyModal-{{ $faculty->id }} .btn-danger:focus{ background: linear-gradient(135deg, rgba(255,235,235,.88), rgba(255,245,245,.46)); box-shadow:0 4px 10px rgba(203,55,55,.15); color: var(--sv-danger); }
        #revokeFacultyModal-{{ $faculty->id }} .btn-light{ background:#fff; border:none; color:#000; transition: all .2s ease; display:inline-flex; align-items:center; gap:.5rem; padding:.5rem 1rem; border-radius:.375rem; }
        #revokeFacultyModal-{{ $faculty->id }} .btn-light i, #revokeFacultyModal-{{ $faculty->id }} .btn-light svg { stroke:#000; }
        #revokeFacultyModal-{{ $faculty->id }} .btn-light:hover, #revokeFacultyModal-{{ $faculty->id }} .btn-light:focus{ background: linear-gradient(135deg, rgba(225,225,225,.88), rgba(240,240,240,.46)); box-shadow:0 4px 10px rgba(0,0,0,.08); color:#000; }
        #revokeFacultyModal-{{ $faculty->id }} .text-truncate-2 { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; overflow: hidden; }
        /* bounce feedback when hide prevented */
        #revokeFacultyModal-{{ $faculty->id }}.modal-static .modal-dialog { transform: scale(1.02); transition: transform .2s ease-in-out; }
        #revokeFacultyModal-{{ $faculty->id }}.modal-static .modal-content { box-shadow: 0 8px 24px rgba(0,0,0,.12), 0 4px 12px rgba(0,0,0,.08); }
      </style>

      <div class="modal-header">
        <h5 class="modal-title fw-semibold d-flex align-items-center gap-2" id="revokeFacultyModalLabel-{{ $faculty->id }}">
      ...
      <div class="modal-body">
        <div class="text-center mb-4">
          <div class="d-inline-flex align-items-center justify-content-center bg-danger bg-opacity-10 rounded-circle mb-3" style="width:64px; height:64px;">
            <i data-feather="user-x" class="text-danger" style="width:28px; height:28px;"></i>
          </div>
          <h6 class="fw-semibold mb-2">Confirm Revoke</h6>
          <p class="text-muted mb-0">Are you sure you want to revoke access for this faculty?</p>
        </div>

        <div class="bg-light rounded-3 p-3 mb-2">
          <div class="small text-muted mb-1">You are about to revoke access for:</div>
          <div class="fw-semibold mb-0">{{ $faculty->name }}</div>
        </div>
        ...
        <div class="alert alert-warning border-0 mb-0" style="background: rgba(255, 193, 7, 0.1);">
          <div class="d-flex align-items-start gap-3">
            ...
            <div class="small">
              <div class="fw-medium text-dark">This will:</div>
              <ul class="mb-0 mt-1 ps-3">
                <li>Remove active appointments</li>
                <li>Set status to rejected</li>
                <li>Block system access</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="modal-footer">
        ...
        <form method="POST" 
          ...
          @csrf
          @method('PATCH')
          <button type="submit" class="btn btn-danger"><i data-feather="user-x"></i> Confirm Revoke</button>
        </form>
      </div>
    </div>
  </div>
</div>
{{-- ░░░ END: Revoke Faculty Confirmation Modal (Themed) ░░░ --}}
