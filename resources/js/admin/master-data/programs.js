...
document.addEventListener('click', function (e) {
  ...
  document.getElementById('editProgramDescription').value = btn.dataset.description || '';
  document.querySelector('#editProgramForm').setAttribute('action', `/admin/programs/${btn.dataset.id}`);
  document.getElementById('programEditLabel').textContent = btn.dataset.name;
});
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (row) {
        ...
        // Update only the remaining columns based on structure
        if (numbered) {
          // 0 = number (preserve), 1=name, 2=code, 3=created, 4=actions
          if (cells[1]) cells[1].textContent = data.program.name;
          if (cells[2]) cells[2].textContent = data.program.code;
          if (cells[3]) cells[3].textContent = data.program.created_at ?? '';
          if (cells[4]) cells[4].innerHTML = actionsHtml(data.program);
        } else {
          // 0=name, 1=code, 2=created, 3=actions
          if (cells[0]) cells[0].textContent = data.program.name;
          if (cells[1]) cells[1].textContent = data.program.code;
          if (cells[2]) cells[2].textContent = data.program.created_at ?? '';
