...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderRows(items) {
    ...
    const filtered = q
      ? items.filter(it => `${it.code || ''} ${it.title || ''} ${it.description || ''}`.toLowerCase().includes(q))
      : items;
    ...
    const rowsHtml = filtered.map(it => {
      ...
      return `
        <tr data-iga-id="${it.id}" data-title="${escapeAttr(it.title || '')}" data-description="${escapeAttr(it.description || '')}" data-code="${escapeAttr(it.code || '')}">
          <td class="iga-code text-nowrap">${escapeHtml(code)}</td>
          <td class="iga-title text-wrap">${escapeHtml(title)}</td>
  ...
  addModalEl?.addEventListener('shown.bs.modal', () => { clearAddErrors(); document.getElementById('igaTitle')?.focus(); });
