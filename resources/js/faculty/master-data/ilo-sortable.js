// -----------------------------------------------------------------------------
// * File: resources/js/faculty/master-data/ilo-sortable.js
// * Description: Drag-and-drop reorder for ILO table + AJAX dropdown loader (Faculty Version)
// -----------------------------------------------------------------------------
// 📜 Log:
...
function initSortableForTable(tbody, saveBtn) {
  ...
  newSaveBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/faculty/master-data/reorder/ilo', {
        method: 'POST',
        headers: {
...
function rowHtml(ilo) {
  ...
  const deleteAction = `
    <form method="POST" action="/faculty/master-data/ilo/${ilo.id}" class="d-inline" onsubmit="return confirm('Delete this ILO?')">
...
function bootIloFilterAjax() {
  ...
  const filterForm = (() => {
    ...
    // Fallback: find a form that posts to /faculty/master-data (GET) and has course_id select
  ...
  filterForm.addEventListener('submit', async (e) => {
    ...
    try {
      const url = `/faculty/master-data/ilos?course_id=${encodeURIComponent(courseId)}`;
