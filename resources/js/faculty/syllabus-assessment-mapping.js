document.addEventListener('DOMContentLoaded', function() {
		// AI Map button: collect snapshot, request auto-map, render, then save
		const aiMapBtn = document.getElementById('svAiAutoMapBtn');
		if (aiMapBtn) {
			aiMapBtn.addEventListener('click', async function() {
				try {
					const syllabusId = document.querySelector('[data-syllabus-id]')?.dataset.syllabusId;
					if (!syllabusId) return;
					// Build a compact snapshot from visible partials
					const parts = Array.from(document.querySelectorAll('.sv-partial'));
					const MAX_SECTION = 900; const MAX_TOTAL = 6000;
					const blocks = [];
					parts.forEach(p => {
						const key = p.getAttribute('data-partial-key') || 'unknown';
						let text = (p.textContent || '').replace(/\s+/g,' ').trim();
						if (!text) return;
						if (text.length > MAX_SECTION) text = text.slice(0, MAX_SECTION);
						blocks.push(`# ${key}\n${text}`);
					});
					let snapshot = blocks.join("\n\n");
					if (snapshot.length > MAX_TOTAL) snapshot = snapshot.slice(0, MAX_TOTAL) + "\n[Snapshot truncated]";

					const resp = await fetch(`/faculty/syllabi/${syllabusId}/assessment-mappings/ai-map`, {
						method: 'POST',
						headers: {
					...
					if (!data.success || !Array.isArray(data.mappings)) {
						console.warn('AI Map failed', data);
						return;
					}
					// Only mark 'x' on existing grid, do not add/remove rows or columns
					...
					if (!weekTable || !distributionTable) return;

					const headerRow = weekTable.querySelector('tr:first-child');
					const headerWeeks = Array.from(headerRow.querySelectorAll('th.week-number')).map(th => parseInt(th.textContent.trim())).filter(n => !isNaN(n));
					...
					// Ensure each existing week row has inline spans per header for marking
					weekRows.forEach(row => {
						const cell = row.querySelector('.week-mapping');
						if (!cell) return;
						const spans = Array.from(cell.querySelectorAll('span'));
						const deficit = headerWeeks.length - spans.length;
						for (let i = 0; i < deficit; i++) {
							const s = document.createElement('span');
							s.style.cssText = 'display:inline-block; width:1.8rem; text-align:center;';
							cell.appendChild(s);
						}
					});

					// Apply mappings by row index if available; otherwise by attempting to match distribution text
					data.mappings.forEach((m, idx) => {
						let targetWeekRow = weekRows[idx] || null;
						let targetDistRow = distRows[idx] || null;

						// Try to find row by distribution name match if index out of range
						if (!targetWeekRow || !targetDistRow) {
							if (m.distribution) {
								const matchIndex = distRows.findIndex(r => {
									const inp = r.querySelector('input.distribution-input');
									return inp && inp.value && inp.value.trim().toLowerCase() === m.distribution.trim().toLowerCase();
								});
								if (matchIndex >= 0) {
									targetWeekRow = weekRows[matchIndex] || targetWeekRow;
									targetDistRow = distRows[matchIndex] || targetDistRow;
								}
							}
						}
						if (!targetWeekRow) return;
						const cell = targetWeekRow.querySelector('.week-mapping');
						if (!cell) return;
						const spans = Array.from(cell.querySelectorAll('span'));
						// Clear existing marks before applying
						spans.forEach(s => { s.textContent = ''; });
						// Mark weeks
						headerWeeks.forEach((w, wIdx) => {
							if (Array.isArray(m.weeks) && m.weeks.includes(w)) {
								if (spans[wIdx]) spans[wIdx].textContent = 'x';
							}
						});
					});

					// Optional: trigger a save if desired, but as requested we only mark the map, not modify rows/columns
				} catch (e) {
					console.warn('AI Map exception', e);
				}
			});
		}
