...
(function(){
  ...
  const promptFns = {
    ...
    ilo_iga_mapping: (snap) => [
      ...
      '- If there are NO ILOs with any task alignments in AMD, DO NOT generate the mapping. Output: {"error": "No ILOs with task alignments found. Please ensure tasks are properly aligned to ILOs first."}',
      ...
      'OUTPUT REQUIREMENTS:',
      'You MUST output ONLY valid JSON in a ```json``` fenced code block. No other text before or after.',
      '- If any validation rule above is triggered, return ONLY the specified error JSON and do not generate mappings or columns.',
      '',
      'JSON STRUCTURE:',
      '{',
      ...
      '- If both ILO rows and IGA columns would be empty after filtering, return the validation error and do not generate mappings',
    ...
    ilo_cdio_sdg_mapping: (snap) => [
      'Generate an ILO-CDIO-SDG mapping that shows task codes for each ILO-CDIO combination and ILO-SDG combination.',
      '',
      'CRITICAL VALIDATION - CHECK BEFORE GENERATING:',
      '- If there are NO ILOs with any alignment to tasks, DO NOT generate the mapping. Output: {"error": "No ILOs with task alignments found. Please ensure tasks are properly aligned to ILOs first."}',
      '- If there are NO CDIOs or NO SDGs in the syllabus, DO NOT generate the mapping. Output: {"error": "No CDIOs or SDGs defined. Please define CDIO and SDG skills first."}',
      '- If ILOs exist but have NO task alignments in TLA, respond with: {"error": "Cannot generate ILO-CDIO-SDG mapping because there are no task alignments in the TLA Activities table. Please ensure tasks are properly aligned to ILOs in the TLA Activities section first."}',
      '- After processing: If there are NO ILO rows with any task alignments, DO NOT generate the mapping. Output: {"error": "No ILO rows have task alignments after filtering."}',
      '- After processing: If there are NO CDIO columns with any task mappings, DO NOT generate the mapping. Output: {"error": "No CDIO columns have task mappings after filtering."}',
      '- After processing: If there are NO SDG columns with any task mappings, DO NOT generate the mapping. Output: {"error": "No SDG columns have task mappings after filtering."}',
      '- If any combination of the above conditions occur (no ILO rows AND/OR no CDIO columns AND/OR no SDG columns with alignments), output the appropriate error message.',
      '- Only proceed with full JSON mapping if there is at least one ILO row with task alignments AND at least one CDIO column with task mappings AND at least one SDG column with task mappings.',
      '',
      'EXISTING MAPPINGS HANDLING:',
      '- If the snapshot contains existing ILO-CDIO-SDG mappings, IGNORE them completely.',
      '- Always REGENERATE the mappings from scratch based on the current TLA Activities and AMD data.',
      '- Do NOT preserve or reference any existing mapping values.',
      '- The generated mappings will REPLACE any existing mappings in the database.',
      '',
      'INPUT: You will receive snapshots containing:',
      '1. ILOs (Intended Learning Outcomes) from the syllabus',
      '2. CDIO skills (institutional codes)',
      '3. SDG goals (institutional codes)',
      '4. TLA Activities table showing task codes aligned to ILOs',
      '5. Assessment Method & Distribution (AMD) with task codes',
      '',
      'TASK MAPPING PROCESS (Follow these steps in order):',
      'Step 0: From AMD, list all tasks and their codes EXCEPT the category rows (ignore category labels).',
      'Step 1: Scan the TLA Activities "Topics/Reading List" column for task names (e.g., "Midterm Exam", "Final Exam", "Quiz #1").',
      'Step 2: For each task, look it up in AMD to get its task code and check which AMD ILO columns have values — AMD ILO columns are the source of truth for task→ILO alignment. Skip tasks with no ILO value in AMD.',
      'Step 3: In TLA, confirm which ILO columns that task row aligns with; keep only those also aligned in AMD.',
      'Step 4: For CDIO mapping, read the CDIO titles/descriptions and choose the best CDIO column for the task using the TLA CDIO column marks; prefer the column where TLA marks the task and the CDIO description best fits.',
      'Step 5: For SDG mapping, read the SDG titles/descriptions and choose the best SDG column for the task using the TLA SDG column marks; prefer the column where TLA marks the task and the SDG description best fits.',
      'Step 6: Use the AMD task code for all mappings; create entries based on BOTH AMD ILO columns AND TLA Activities ILO/CDIO/SDG columns.',
      '',
      'TASK OUTPUT:',
      'For each ILO, create a row with:',
      '- CDIO columns: comma-separated task codes for each ILO-CDIO pair (based on TLA Activities CDIO alignment)',
      '- SDG columns: comma-separated task codes for each ILO-SDG pair (based on TLA Activities SDG alignment)',
      '- Column references use numeric keys (1, 2, 3, etc.) representing column positions',
      '',
      'OUTPUT REQUIREMENTS:',
      'You MUST output ONLY valid JSON in a ```json``` fenced code block. No other text before or after.',
      '',
      ...
      '    {',
      '      "ilo_text": "ILO1",',
      '      "cdios": {"1": "FE,ME", "2": "ME", "3": null},',
      '      "sdgs": {"1": "FE,ME", "2": "ME", "3": null},',
      '      "position": 0',
      '    },',
      '    {',
      '      "ilo_text": "ILO2",',
      '      "cdios": {"1": "ME", "2": "FE,ME", "3": "FE"},',
      '      "sdgs": {"1": null, "2": null, "3": null},',
      '      "position": 1',
      '    }',
      '  ]',
      '}',
      ...
      '- mappings: Array of ILO mapping rows:',
      '  * ilo_text: The ILO label as string (e.g., "ILO1", "ILO2")',
      '  * cdios: JSON object where keys are numeric column positions ("1", "2", "3", etc.) and values are:',
      '    - Comma-separated task codes (no spaces) if tasks align to that CDIO column for this ILO',
      '    - null if no tasks align to that CDIO column for this ILO',
      '  * sdgs: JSON object where keys are numeric column positions ("1", "2", "3", etc.) and values are:',
      '    - Comma-separated task codes (no spaces) if tasks align to that SDG column for this ILO',
      '    - null if no tasks align to that SDG column for this ILO',
      '  * position: Integer representing ILO row order (0-indexed)',
      '',
      'MAPPING RULES:',
      '- STEP-BY-STEP PROCESS:',
      '  1. Find all tasks in TLA Activities "Topics/Reading List" column',
      '  2. For each task, look it up in AMD to get task code and check which AMD ILO columns have values',
      '  3. Return to TLA Activities to check which ILO, CDIO, and SDG columns that task aligns with',
      '  4. For CDIO columns: Use column position numbers (1, 2, 3, etc.) as keys in the cdios object',
      '  5. For SDG columns: Use column position numbers (1, 2, 3, etc.) as keys in the sdgs object',
      '  6. Use task code from AMD for all mappings',
      '- Determine the TOTAL number of CDIO and SDG columns from the TLA Activities table (count how many exist)',
      '- For each ILO in the syllabus:',
      '  1. BEFORE creating a mapping entry: Check if this ILO has ANY task alignments. If an ILO has NO alignments at all, SKIP it and do not create a row for it.',
      '  2. Only create a mapping entry for ILOs that have at least one task alignment',
      '  3. In cdios object: Create entries for each CDIO column position (1, 2, 3, etc.):',
      '     - List task codes (from AMD) for tasks that align to BOTH this ILO (in TLA) AND that CDIO column (in TLA)',
      '     - Use null if no tasks align to that column',
      '  4. In sdgs object: Create entries for each SDG column position (1, 2, 3, etc.):',
      '     - List task codes (from AMD) for tasks that align to BOTH this ILO (in TLA) AND that SDG column (in TLA)',
      '     - Use null if no tasks align to that column',
      '- Separate multiple task codes with "," (comma NO space)',
      '- Use null (not empty string or "-") for empty cells',
      '- Task codes come from AMD (e.g., "ME", "FE", "QCT", "ARR", "PR", "LE", "LEX")',
      '- DO NOT generate rows for ILOs with no alignments',
      '',
      'EXAMPLE OUTPUT:',
      '{',
      ...
      '- Output ONLY the JSON block; no explanation, commentary, or preamble',
      '- ilo_text should be the ILO label only (e.g., "ILO1", not full ILO description)',
      '- cdios and sdgs keys are NUMERIC STRINGS ("1", "2", "3", etc.) representing column positions, NOT CDIO/SDG codes',
      '- Both cdios and sdgs must have the SAME set of numeric keys (e.g., if there are 3 CDIO columns and 3 SDG columns, both objects should have keys "1", "2", "3")',
      '- Task codes should be comma-separated WITHOUT spaces (e.g., "FE,ME" not "FE, ME")',
      '- Use null (not "" or "-") for empty cells in cdios and sdgs fields',
      '- Position values must be sequential starting from 0 matching the ILO order',
      '- Do not include any text outside the json code block',
      '- Follow the exact structure above'
    ].join(' '),
