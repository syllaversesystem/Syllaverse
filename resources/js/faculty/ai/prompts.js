...
(function(){
  ...
  const promptFns = {
    ...
    ilo_so_cpa_mapping: (snap) => [
      ...
      '- SO columns: comma-separated task codes for each ILO-SO pair',
      '- C column: comma-separated task codes with Cognitive domain',
      '- P column: comma-separated task codes with Psychomotor domain',
      '- A column: comma-separated task codes with Affective domain',
      '',
      'OUTPUT REQUIREMENTS:',
      'You MUST output ONLY valid JSON in a ```json``` fenced code block. No other text before or after.',
      '',
      ...
      '  1. BEFORE creating a mapping entry: Check if this ILO has ANY task alignments (in TLA Activities or AMD). If an ILO has NO alignments at all, SKIP it and do not create a row for it.',
      '  2. Only create a mapping entry for ILOs that have at least one task alignment',
      '  3. In sos object, for each SO in so_columns: list task codes that align to BOTH this ILO AND that SO (use null if empty)',
      '  4. In c field: list all task codes for this ILO that have Cognitive domain in AMD',
      '  5. In p field: list all task codes for this ILO that have Psychomotor domain in AMD',
      '  6. In a field: list all task codes for this ILO that have Affective domain in AMD',
